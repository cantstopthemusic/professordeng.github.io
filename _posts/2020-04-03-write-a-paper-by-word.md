...
2. 摘要是要放在目录里的，目录是根据样式的多级标题来编排的。
   ...
   如果要修改标题 1 的样式，选中标题 1→鼠标右键→修改→名称改为摘要，相应的样式随学校要求
...
1. 正文的标题样式设置
   ...
   开始栏→多级列表→定义新的多级样式（注意不是 “定义新的多级列表”），其中 “输入编号的格式” 中可以在数字 n 前面或后面添加描述，例如 “n” 可以改为 “第 n 章”，然后 n 的样式可以在 “编号样式” 中修改。在 “输入编号的格式” 边上有个 “字体” 按钮，可以修改多级列表的字体，包括字体（黑体等）、字形（加粗等）以及字号（四号等）。
